 ...
 * @author Marius Adam
 */
public class MatrixThreadedTransformer<T> extends MatrixDirectTransformer<T> {
    private Integer threadsCount;
    private Matrix<T> m1;
    private Matrix<T> m2;
    private Matrix<T> result;

    public MatrixThreadedTransformer(BiFunction<T, T, T> basicOperation, Function<Dimension, Matrix<T>> matrixFactory) {
    ...
    public Runnable getJob(Integer start, Integer stop) {
        return () -> {
            for (int i = start; i < stop; i++) {
                result.set(
                        i,
                        getBasicOperation().apply(m1.get(i), m2.get(i))
                );
            }
        };
    }

    public MatrixThreadedTransformer setThreadsCount(Integer threadsCount) {
